...
train_loader, val_loader, class_name_to_idx = create_dataloaders(images_array, bounding_boxes_array, keypoints_array, 
                                                                 classes_array, batch_size=32, resize_to=[256, 256], 
                                                                 transforms=None)
...
#train_loader = torch.utils.data.DataLoader(train_loader, batch_size=12, shuffle=True)  # Re-create DataLoader for the subset
#val_loader = torch.utils.data.Subset(val_loader.dataset, range(int(len(val_loader.dataset) * 0.05)))
#val_loader = torch.utils.data.DataLoader(val_loader, batch_size=12, shuffle=True)  # Re-create DataLoader for the subset
