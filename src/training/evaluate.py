...
def evaluate_model(val_loader, model, class_name_to_idx, log_dir="logs/val_logs", num_epoch=0, timestamp='', autocast_enabled=True):
    ...
    with torch.no_grad():  # Disable gradient computation during evaluation
        for batch_idx, (images, targets) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            total_loss = classification_loss + keypoints_loss + bbox_loss 
            ...
            if batch_idx == 0:
                ...
                for i in range(4):
                    ...
                    # Log heatmaps in TensorBoard
                    writer.add_image(f"BBox_Heatmaps_Pred/{i}", bbox_pred_tensor, num_epoch, dataformats="CHW")
                    writer.add_image(f"BBox_Heatmaps_Target/{i}", bbox_target_tensor, num_epoch, dataformats="CHW")
    ...
    print(f"Val BBox Accuracy: {val_bbox_accuracy:.6f}, Val Keypoints Accuracy: {val_keypoints_accuracy:.6f}")
